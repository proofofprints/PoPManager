...
Common issues and solutions for OverManager Cloud Sync.
...
1. **Verify the cloud API is reachable:**
   ```
   curl https://cloud.overbuildlabs.com/health
   ```
...
2. **Check your internet connection:**
   - Can you open [cloud.overbuildlabs.com](https://cloud.overbuildlabs.com) in a browser?
   - Is OverManager behind a corporate proxy that blocks outbound WebSocket connections?

3. **Check OverManager logs:**
...
**Fix:**
1. Try logging in at [cloud.overbuildlabs.com](https://cloud.overbuildlabs.com) first to verify your credentials work
...
**Cause:** Your refresh token expired because OverManager wasn't opened for 30+ days.
...
4. **Check OverManager logs** for `cloud sync` lines — they'll show whether pushes are succeeding or failing
...
2. Wait — OverManager will automatically retry with exponential backoff (30s → 1m → 5m → 30m)
3. If the queue keeps growing for hours, check the OverManager log for specific error messages
...
3. OverManager automatically prunes: snapshots older than 30 days and alerts older than 90 days are dropped from the queue
...
**Cause:** OverManager desktop is offline or the WebSocket connection is down.

**Fix:**
1. Open OverManager on the desktop — it will connect to the cloud and pick up pending commands
...
**Cause:** OverManager received the command but couldn't execute it.

**Common reasons:**
- The target miner is offline or unreachable on the LAN
- The miner rejected the configuration (wrong pool URL format, etc.)
- The target miner was removed from OverManager since the command was queued
...
**Fix:** Subscribe at [cloud.overbuildlabs.com](https://cloud.overbuildlabs.com) → Account → Subscription.
...
**Cause:** The API key for this instance was regenerated from the web portal or another OverManager installation.

**Fix:** Sign out and sign in again. A new API key will be issued for this instance.

## Firewall & Network

### OverManager behind a corporate firewall

OverManager Cloud Sync requires outbound access to:

| Destination | Port | Protocol | Purpose |
|---|---|---|---|
| `cloud.overbuildlabs.com` | 443 | HTTPS | REST API calls |
| `cloud.overbuildlabs.com` | 443 | WSS | WebSocket for commands |
...
OverManager currently does not support HTTP proxy configuration for cloud sync. If you're behind a proxy, the cloud connection may fail. This is a known limitation — proxy support is planned for a future release.

## Data & Privacy

### What data does OverManager send to the cloud?
...
- OverManager version and instance name
...
1. Log in to [cloud.overbuildlabs.com](https://cloud.overbuildlabs.com)
...
2. **Open a GitHub issue** — [github.com/overbuildlabs/OverManager/issues](https://github.com/overbuildlabs/OverManager/issues) with:
   - Your OverManager version (Settings → About)
